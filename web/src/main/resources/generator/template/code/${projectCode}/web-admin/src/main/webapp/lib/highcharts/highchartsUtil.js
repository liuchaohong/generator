...
var highchartsUtil = {
		"chart" : function(containerId, obj) {
			...
			var xfieldValueList = new Array();
			for(var i = 0;i<dataList.length;i++){
				var date = dataList[i][x];
				xfieldValueList.push(date);
			}
		...
		"setData" : function(chart,data) {
			...
			if (chartType == "pie"){
				...
				for(var i = 0;i<sers.length;i++){
					var name = sers[i].title;
					var yField = sers[i].y;
					var data = new Array();
					for(var j=0;j<dataList.length;j++){
						var row = dataList[j];
						var yValue = row[yField];
						data.push(yValue);
					}
					var b = {name: name, data: data};
					chart.addSeries(b);
				}
			}
